...
public class RotSlime : Enemy
{
    ...
    private void PlayHurtAnimation()
    {
        ...
        {
            isHurt = true;
            anim.Play("RotHurt"); // Play hurt animation
    ...
    {
        // 等待受伤动画的播放时长
        yield return new WaitForSeconds(0.5f); // 根据你的受伤动画时长调整这个值

        isHurt = false; // 重置受伤状态

        // 可以在这里决定在重置后播放什么动画，比如返回到Idle或其他状态
        if (isGrounded) // 确保角色在地面时返回Idle状态
        {
            anim.SetBool("running", false); // 根据需要设置
        }
        else
        {
            // 如果角色在跳跃，可能不需要切换到Idle
            anim.SetTrigger("jumpTrigger"); // 如果在空中，可以继续跳跃动画
        }
    }
